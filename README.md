# Experiment Tracking App (ETA) | 实验跟踪应用 (ETA)

[English](#english) | [中文](#中文)

---

## English

### Overview
Experiment Tracking App (ETA) is a Django web application for research groups to manage **Projects → Experiments → Flows → Steps**, record equipment/material usage, and keep **traceable step history across experiments**.

### Key Concepts
- **Research Group**: Access boundary for non-staff users. Users see only data in their group.
- **Project**: Has a 3-letter `project_code` (e.g. `SOL`).
- **Experiment (Exp)**: Auto-generated name based on project code + sequence (e.g. `SOL005`).
- **Flow (ExpFlow)**: Two-letter code within an experiment (e.g. `AA`) with full ID `SOL005AA`.
- **Step (ExpStep)**: Two-letter `step_name` + per-flow sequence number (e.g. `MX00`) with full ID `SOL005AA-MX00`.

### Features
- **Auth**: Login/logout, change password.
- **Group-scoped access control**: Non-staff users can access only experiments under their research group.
- **Experiment dashboard**: Pagination + search by experiment name/description/project.
- **Flow management**: Create/delete flows; auto-generated `full_flow` code.
- **Step management**:
  - Create/edit/delete steps
  - Status: `Planned` / `Completed` / `Canceled` (completed timestamp set automatically)
  - Attach equipment/tool, recipe, notes
  - Store materials/components as JSON list (`components`)
  - **Cross-experiment parent linking**: a step can reference any previous step (for sample/history tracing)
- **Step name templates**: Maintain selectable 2-letter step codes with labels/categories (`StepNameTemplate`).
- **Bulk operations**: Bulk update status; multi-select delete; copy steps to another flow (within accessible experiments).
- **Equipment database**: Add/edit/view equipment with owner, location, utilities/specs, active flag.
- **Barcodes**:
  - Auto-generated IDs (`F000001` for flows, `S000001` for steps)
  - Printable pages for flow/step barcodes
  - Utility script to backfill barcodes for existing records
- **APIs (JSON)**:
  - `GET /api/steps/` (group-scoped steps)
  - `GET /api/experiments_with_flows/` (for copy-step dropdown)
- **Global code search**: Search by `full_flow` or `full_step` and redirect to the right experiment page.

### Tech Stack
- Python + Django (settings indicate Django **5.2.x**, generated by 5.2.6)
- SQLite (default `db.sqlite3`)
- Bootstrap/jQuery/select2 (vendored static assets)

---

## 中文

### 概览
实验跟踪应用（ETA）是一个基于 Django 的 Web 应用，用于研究团队管理 **项目 → 实验 → 流程 → 步骤**，记录设备/材料使用情况，并支持 **跨实验的步骤溯源**（通过父步骤关联形成历史链）。

### 核心概念
- **研究组 (Research Group)**：普通用户的数据访问边界，只能看到本组数据。
- **项目 (Project)**：包含 3 位字母的 `project_code`（例如 `SOL`）。
- **实验 (Exp)**：根据项目代码自动生成名称（例如 `SOL005`）。
- **流程 (ExpFlow)**：实验内 2 位字母代码（例如 `AA`），完整编号 `SOL005AA`。
- **步骤 (ExpStep)**：2 位 `step_name` + 流程内序号（例如 `MX00`），完整编号 `SOL005AA-MX00`。

### 功能
- **登录与账户**：登录/退出、修改密码。
- **按研究组隔离数据**：非管理员用户只能访问所属研究组的实验数据。
- **实验仪表盘**：分页展示 + 按实验名/描述/项目名搜索。
- **流程管理**：创建/删除流程；自动生成 `full_flow` 编码。
- **步骤管理**：
  - 新增/编辑/删除步骤
  - 状态：`Planned` / `Completed` / `Canceled`（完成时自动写入完成时间）
  - 记录设备、配方、备注
  - 以 JSON 列表形式记录材料/组件（`components`）
  - **跨实验父步骤关联**：步骤可关联任意历史步骤，用于样品/工艺链追踪
- **步骤模板**：可维护常用两位步骤码、标签与分类（`StepNameTemplate`）。
- **批量操作**：批量改状态、批量删除、复制步骤到其它流程（仅限用户可访问的实验范围）。
- **设备库**：设备新增/编辑/查看，包含负责人、位置、规格/水电气排风等信息、启用状态。
- **条码**：
  - 自动生成条码 ID（流程 `F000001`，步骤 `S000001`）
  - 提供流程/步骤条码打印页面
  - 提供脚本为历史数据补齐条码
- **JSON 接口**：
  - `GET /api/steps/`（按研究组返回步骤列表）
  - `GET /api/experiments_with_flows/`（复制步骤时用于选择目标流程）
- **全局编号搜索**：输入 `full_flow` 或 `full_step`，自动跳转到对应实验并展开流程。

---

## Quick Start (Dev) | 快速开始（开发环境）

> This repo includes a local virtualenv at `exptrack/` in your workspace; for a clean GitHub clone, you can create your own venv.

```zsh
cd experiment_app

# (Option A) use the repo venv if present
../exptrack/bin/python -m pip install --upgrade pip

# (Option B) create your own venv
# python3 -m venv .venv
# source .venv/bin/activate

# install deps (at minimum Django)
# pip install "Django>=5.2,<6"

# database setup
../exptrack/bin/python manage.py migrate

# create admin user
../exptrack/bin/python manage.py createsuperuser

# run dev server
../exptrack/bin/python manage.py runserver
```

Open: `http://127.0.0.1:8000/`

---

## Barcode Backfill Script | 条码补齐脚本

If you already have records without barcodes:

```zsh
cd experiment_app
../exptrack/bin/python manage.py shell < scripts/generate_barcodes.py
```

---

## Deployment Note (uWSGI) | 部署备注（uWSGI）

A sample `uwsgi.ini` is included under `experiment_app/uwsgi.ini` (paths may need adjustment for your server).

---

## License

No license file found in this repository. Add a `LICENSE` if you plan to open source this project.
